...
export function usePrivacy(options: UsePrivacyOptions = {}): UsePrivacyReturn {
  ...
  const grantConsents = useCallback(async (data: ConsentBatchCreate) => {
    setIsLoadingConsent(true);
    setConsentError(null);
    try {
  ...
  const createDeletionRequest = useCallback(async (data: DeletionRequestCreate): Promise<DeletionRequestResponse> => {
    setIsLoadingDeletion(true);
    setDeletionError(null);
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to create deletion request';
      setDeletionError(errorMessage);
      throw err;
    } finally {
      setIsLoadingDeletion(false);
