  ...
  ({ className, label, error, hint, leftIcon, rightIcon, id, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        ...
        <div className="relative">
          {leftIcon && (
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <span className="text-[var(--brand-text-secondary)] sm:text-sm">{leftIcon}</span>
            </div>
          )}
          <input
            ref={ref}
          ...
          {rightIcon && (
            <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
              <span className="text-[var(--brand-text-secondary)] sm:text-sm">{rightIcon}</span>
            </div>
          )}
        </div>
        {error && (
        ...
        {hint && !error && (
          <p id={`${inputId}-hint`} className="mt-1 text-sm text-[var(--brand-text-secondary)]">
            {hint}
          </p>
        )}
      </div>
