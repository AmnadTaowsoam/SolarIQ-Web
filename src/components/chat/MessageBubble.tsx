...
function QuoteCard({
  ...
  quoteData: NonNullable<Message['quoteData']>
  isOwn: boolean
}) {
  return (
    <div
...
export function MessageBubble({ message, showAvatar = true }: MessageBubbleProps) {
  ...
  const [_showTime, setShowTime] = useState(false)
  ...
  const initials = isOwn ? 'Y' : t('customer').charAt(0)
          ...
          >
            ...
            {isOwn && (
              <svg
                className={cn(
                  'w-3.5 h-3.5',
                  message.isRead ? 'text-orange-200' : 'text-orange-300'
                )}
                viewBox="0 0 24 24"
                fill="currentColor"
              >
