...
export function UpgradePrompt({
  ...
  const recommendedPlanId = currentIndex < planOrder.length - 1 ? planOrder[currentIndex + 1] : null
  ...
  const featureText = featureName ? ` to unlock more ${featureName}` : ' for more features'

  if (variant === 'banner') {
    return (
      <div
        className={`bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-4 rounded-lg ${className}`}
      >
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="secondary"
            size="sm"
            className="bg-[var(--brand-surface)] text-blue-600 hover:bg-blue-50"
          >
            Upgrade
          </Button>
        </div>
  ...
  if (variant === 'card') {
    return (
      <div
        className={`bg-[var(--brand-surface)] border border-[var(--brand-border)] rounded-lg p-6 ${className}`}
      >
        <div className="text-center">
          ...
          <p className="text-[var(--brand-text-secondary)] text-sm mb-4">
            Get more{featureText} and unlock premium features.
          </p>
          <div className="text-2xl font-bold text-[var(--brand-text)] mb-4">
            ฿{recommendedPlan.price_thb.toLocaleString()}
            <span className="text-sm font-normal text-[var(--brand-text-secondary)]">/month</span>
          </div>
  ...
  return (
    <div
      className={`flex items-center justify-between p-4 bg-[var(--brand-background)] rounded-lg ${className}`}
    >
      <div className="flex items-center space-x-3">
        ...
        <div>
          <p className="text-sm font-medium text-[var(--brand-text)]">Need more{featureText}?</p>
