...
export function UsageHistoryChart({ className = '', limit = 6 }: UsageHistoryChartProps) {
  ...
  if (isLoading) {
    return (
      <div className={`bg-[var(--brand-surface)] rounded-lg shadow p-6 ${className}`}>
        <h3 className="text-lg font-semibold text-[var(--brand-text)] mb-4">Usage History</h3>
        <div className="animate-pulse space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center space-x-4">
              <div className="h-4 bg-[var(--brand-border)] rounded w-24"></div>
              <div className="h-4 bg-[var(--brand-border)] rounded w-16"></div>
              <div className="h-4 bg-[var(--brand-border)] rounded flex-1"></div>
            </div>
          ))}
        </div>
      </div>
  ...
  const groupedByPeriod = history.reduce(
    (acc, item) => {
  ...
  const maxValue = Math.max(...history.map((h) => h.usage_count), 1)
      ...
      <div className="space-y-6">
        {periods.map((period) => (
          <div key={`${period.period_start}-${period.period_end}`}>
            <h4 className="text-sm font-medium text-[var(--brand-text-secondary)] mb-2">
              ...
              {period.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center space-x-3">
                    <span className="text-xs text-[var(--brand-text-secondary)] w-20 truncate">
                      {config.displayName}
                    </span>
                    <div className="flex-1 bg-[var(--brand-background)] rounded-full h-4 overflow-hidden">
                      <div
                    ...
                    <span className="text-xs font-medium text-[var(--brand-text)] w-12 text-right">
                      {item.usage_count}
                    </span>
                  </div>
                )
