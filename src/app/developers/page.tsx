  ...
  return (
    ...
    >
      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-bold ${
        done ? 'bg-green-500 text-white' : 'bg-orange-100 text-orange-600 group-hover:bg-orange-500 group-hover:text-white transition-colors'
      }`}>
        {done ? (
      ...
      <div>
        <p className="text-sm font-semibold text-gray-900 group-hover:text-orange-600 transition-colors">{title}</p>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-gray-400 text-sm">Developer Portal</p>
            <h2 className="text-2xl font-bold mt-1">ยินดีต้อนรับนักพัฒนา!</h2>
            <p className="text-gray-400 text-sm mt-2">
              เชื่อมต่อ SolarIQ กับระบบของคุณผ่าน REST API
            </p>
          </div>
          <div className="hidden md:block">
            <svg className="w-16 h-16 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
            </svg>
          </div>
        </div>
      </div>
      ...
      {/* Quick stats */}
      <div>
        <h3 className="text-sm font-semibold text-gray-700 mb-3">สรุปการใช้งานวันนี้</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard
            label="API Calls วันนี้"
            value={usage?.totalCallsToday.toLocaleString() ?? '—'}
            sub="จาก 10,000 req/วัน"
          />
          <StatCard
            label="Success Rate"
            value={usage ? `${usage.successRate}%` : '—'}
            sub="เฉลี่ย 30 วันที่ผ่านมา"
          />
          <StatCard
            label="Avg Latency"
            value={usage ? `${usage.avgLatencyMs} ms` : '—'}
            sub="ค่าเฉลี่ยตอบสนอง"
          />
        </div>
      </div>
      ...
      {/* Getting started */}
      <div>
        <h3 className="text-sm font-semibold text-gray-700 mb-3">เริ่มต้นใช้งาน</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <StepCard
            step={1}
            title="สร้าง API Key"
            description="สร้าง key สำหรับ Live หรือ Test environment"
          ...
          <StepCard
            ...
            description="ศึกษา endpoints, parameters และ responses"
            href="/developers/keys"
            done={false}
          />
          <StepCard
            step={3}
            title="ทดสอบใน Sandbox"
            description="ทดสอบ API ด้วยข้อมูลตัวอย่างก่อน go live"
      ...
      {/* Quick links */}
      <div>
        <h3 className="text-sm font-semibold text-gray-700 mb-3">ลิงก์ด่วน</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <QuickLinkCard
            href="/developers/keys"
            label="จัดการ API Keys"
            description="สร้าง แก้ไข และเพิกถอน API Keys"
            icon={
          ...
          <QuickLinkCard
            ...
            label="ดูสถิติการใช้งาน"
            description="ติดตาม API calls และ quota"
            icon={
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <QuickLinkCard
            href="/developers/sandbox"
            label="เปิด Sandbox"
            description="ทดสอบ API ด้วยข้อมูลตัวอย่าง"
            icon={
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
