...
function CreateKeyModal({
  ...
  const ALL_PERMISSIONS = [
    { id: 'leads:read', label: t('keys.permissions.leadsRead') },
    { id: 'leads:write', label: t('keys.permissions.leadsWrite') },
    { id: 'solar:analyze', label: t('keys.permissions.solarAnalyze') },
    { id: 'proposals:read', label: t('keys.permissions.proposalsRead') },
    { id: 'proposals:write', label: t('keys.permissions.proposalsWrite') },
    { id: 'deals:read', label: t('keys.permissions.dealsRead') },
    { id: 'deals:write', label: t('keys.permissions.dealsWrite') },
    { id: 'webhooks:manage', label: t('keys.permissions.webhooksManage') },
  ]
          ...
          {createdKey ? (
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-semibold text-green-800">
                    {t('keys.createModal.successMessage')}
                  </p>
                </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                  ...
                  >
                    {copied ? t('keys.createModal.copied') : t('keys.createModal.copy')}
                  </button>
                </div>
              </div>
              <button
            ...
            <>
                  ...
                  {(['test', 'live'] as const).map((env) => (
                    ...
                    >
                      <div className="flex items-center gap-2 justify-center">
                        <div
                          className={cn(
                            ...
                            env === 'live' ? 'bg-orange-500' : 'bg-blue-500'
                          )}
                        />
                        {env === 'live' ? 'Live' : 'Test'}
                      </div>
                      <div className="text-xs opacity-70 mt-0.5">
                        {env === 'live'
                          ? t('keys.createModal.liveData')
                          : t('keys.createModal.testData')}
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isCreating ? t('keys.createModal.creating') : t('keys.createModal.create')}
                </button>
              </div>
            </>
          )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-blue-800">{t('keys.securityNote.title')}</p>
          <p className="text-xs text-blue-700 mt-0.5">{t('keys.securityNote.message')}</p>
        </div>
      </div>

      {/* Modals */}
      {showCreateModal && (
        <CreateKeyModal onClose={() => setShowCreateModal(false)} onCreate={createKey} t={t} />
      )}
      {revokeTarget && (
        <RevokeConfirmModal
