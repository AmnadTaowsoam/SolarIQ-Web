...
function AddEndpointModal({
  ...
  const validateUrl = (value: string) => {
    try {
      ...
      setUrlError('URL ไม่ถูกต้อง กรุณาใส่ URL ที่ขึ้นต้นด้วย https://')
      return false
    }
  }

  const handleAdd = async () => {
    if (!validateUrl(url) || selectedEvents.length === 0) return
    setIsCreating(true)
    try {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 className="text-base font-bold text-gray-900">
            {createdWebhook ? 'Webhook สร้างแล้ว' : 'เพิ่ม Webhook Endpoint'}
          </h3>
          ...
          {createdWebhook ? (
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                <p className="text-sm font-semibold text-green-800 mb-1">Webhook สร้างสำเร็จ!</p>
                <p className="text-xs text-green-700">บันทึก secret ด้านล่างไว้ใช้ยืนยัน signature</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-600 mb-1.5">Webhook Secret</p>
                <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                  <code className="text-xs font-mono text-gray-800 break-all">{createdWebhook.secret}</code>
                </div>
                <p className="text-xs text-gray-500 mt-1.5">ใช้ secret นี้เพื่อยืนยัน HMAC signature ใน header X-SolarIQ-Signature</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-600 mb-1.5">Events ที่เลือก ({createdWebhook.events.length} รายการ)</p>
                <div className="flex flex-wrap gap-1.5">
                  {createdWebhook.events.map((e) => (
                    <code key={e} className="text-xs bg-orange-50 text-orange-700 px-2 py-1 rounded-lg font-mono">{e}</code>
                  ))}
                </div>
              </div>
            ...
            <>
              <div>
                ...
                <input
                  ...
                  className={cn(
                    ...
                    urlError ? 'border-red-300 focus:ring-red-400' : 'border-gray-200 focus:ring-orange-400'
                  )}
                />
                    ...
                    <label key={event.id} className="flex items-start gap-3 cursor-pointer group">
                      <div
                        className={cn(
                          ...
                          selectedEvents.includes(event.id) ? 'bg-orange-500 border-orange-500' : 'border-gray-300 group-hover:border-orange-400'
                        )}
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={onClose} className="flex-1 py-3 border border-gray-200 text-sm font-medium text-gray-700 rounded-xl hover:bg-gray-50 transition-colors">ยกเลิก</button>
                <button
                  onClick={handleAdd}
                ...
                >
                  {isCreating ? 'กำลังสร้าง...' : 'เพิ่ม Endpoint'}
                </button>
              </div>
            </>
          )}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">รับการแจ้งเตือนแบบ real-time เมื่อเกิด events</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-gray-500">ยังไม่มี Webhook Endpoint</p>
            <button onClick={() => setShowAddModal(true)} className="mt-3 text-sm text-orange-600 font-medium hover:text-orange-700">
              เพิ่ม Endpoint แรก
            </button>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="border-b border-gray-100">
                <tr>
                  {['Endpoint URL', 'Events', 'สถานะ', 'Triggered ล่าสุด', 'จัดการ'].map((h) => (
                ...
                {webhooks.map((wh) => (
                  <tr key={wh.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-5 py-4">
                      ...
                      {wh.failureCount > 0 && <p className="text-xs text-red-600 mt-0.5">{wh.failureCount} failures</p>}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm font-medium text-gray-700">{wh.events.length} events</span>
                        <div className="group relative">
                          ...
                          <div className="absolute left-0 top-full mt-1 w-48 bg-white border border-gray-200 rounded-xl shadow-lg p-2 hidden group-hover:block z-10">
                            {wh.events.map((e) => (
                              <code key={e} className="block text-xs text-gray-700 py-0.5">{e}</code>
                            ))}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-5 py-4">
                      <span className={cn(
                        ...
                        wh.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                      )}>
                        <span className={cn('w-1.5 h-1.5 rounded-full', wh.status === 'active' ? 'bg-green-500' : 'bg-gray-400')} />
                        {wh.status === 'active' ? 'ใช้งานอยู่' : 'ปิดใช้งาน'}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-xs text-gray-500 whitespace-nowrap">
                      {wh.lastTriggeredAt
                        ? formatDistanceToNow(new Date(wh.lastTriggeredAt), { addSuffix: true, locale: th })
                        : 'ยังไม่เคยถูก trigger'}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={cn(
                            ...
                            testResult[wh.id] === true ? 'border-green-300 text-green-700 bg-green-50' :
                            testResult[wh.id] === false ? 'border-red-300 text-red-700 bg-red-50' :
                            'border-blue-200 text-blue-700 hover:bg-blue-50'
                          )}
                        >
                          {testingId === wh.id ? 'กำลังส่ง...' :
                           testResult[wh.id] === true ? 'ส่งแล้ว!' :
                           testResult[wh.id] === false ? 'ส่งไม่ได้' : 'ทดสอบ'}
                        </button>
                        <button
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-gray-700">ยืนยัน Webhook</p>
          <p className="text-xs text-gray-500 mt-0.5">
            SolarIQ ส่ง signature ใน header <code className="font-mono bg-gray-100 px-1 rounded">X-SolarIQ-Signature</code> เพื่อยืนยันความถูกต้องของ payload
          </p>
        </div>
      </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            <h3 className="text-base font-bold text-gray-900 text-center mb-2">ลบ Webhook</h3>
            <p className="text-sm text-gray-600 text-center mb-6">
              ยืนยันการลบ endpoint <strong className="break-all">{deleteTarget.url}</strong>?
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2.5 border border-gray-200 text-sm font-medium text-gray-700 rounded-xl hover:bg-gray-50 transition-colors">ยกเลิก</button>
              <button
                onClick={async () => { await deleteWebhook(deleteTarget.id); setDeleteTarget(null) }}
