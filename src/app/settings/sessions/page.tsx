...
function deviceIcon(device: string): React.ReactNode {
  ...
  if (lower.includes('iphone') || lower.includes('android') || lower.includes('galaxy') || lower.includes('pixel')) {
    return (
...
function SessionCard({
  ...
  return (
    <div
      className={clsx(
        'bg-white rounded-xl border-2 p-5 transition-shadow hover:shadow-md',
        session.is_current ? 'border-green-400 ring-1 ring-green-100' : 'border-gray-200'
      )}
    >
      <div className="flex items-start gap-4">
        ...
        <div
          className={clsx(
            ...
            session.is_current ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'
          )}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-600 mt-1">{session.device}</p>
